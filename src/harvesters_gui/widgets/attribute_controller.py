...
from harvesters_gui.widgets.feature_tree import (
    FeatureEditDelegate,
    FilterProxyModel,
    FeatureTreeModel,
)
from harvesters_gui.utils import get_system_font, compose_tooltip


class ActionExpandAll(Action):
    def __init__(self, icon=None, title="", parent=None, action=None):
        super().__init__(icon=icon, title=title, parent=parent, action=action)
