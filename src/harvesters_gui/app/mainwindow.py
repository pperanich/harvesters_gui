...
from harvesters_gui.utils import compose_tooltip
from harvesters_gui.widgets.about import About
from harvesters_gui.widgets.action import Action
from harvesters_gui.widgets.attribute_controller import (
    AttributeController,
)
from harvesters_gui.widgets.device_list import ComboBoxDeviceList
from harvesters_gui.widgets.display_rate_list import (
    ComboBoxDisplayRateList,
)
from harvesters_gui.utils import get_system_font
from harvesters_gui.widgets.icon import Icon
from harvesters_gui.widgets.thread import _PyQtThread
...
class Harvester(QMainWindow):
    ...
    def _initialize_gui_toolbar(self, observers):
        ...
        button_connect.setToolTip(
            compose_tooltip("Connect the selected device to Harvester", shortcut_key)
        )
        ...
        button_disconnect.setToolTip(
            compose_tooltip("Disconnect the device from Harvester", shortcut_key)
        )
        ...
        button_about.setToolTip(compose_tooltip("Show information about Harvester"))
        button_about.toggle()
        ...
        group_gentl_info.actionTriggered[QAction].connect(self.on_button_clicked_action)
        group_connection.actionTriggered[QAction].connect(self.on_button_clicked_action)
        group_device.actionTriggered[QAction].connect(self.on_button_clicked_action)
        group_display.actionTriggered[QAction].connect(self.on_button_clicked_action)
        group_help.actionTriggered[QAction].connect(self.on_button_clicked_action)
    ...
    def action_on_connect(self):
        ...
        try:
            if self.ia.remote_device.node_map:
                self._widget_attribute_controller = AttributeController(
                    self.ia.remote_device.node_map, parent=self
                )
    ...
    def is_enabled_on_start_image_acquisition(self):
        ...
        if self.files:
            if self.ia:
                if not self.ia.is_acquiring() or self.canvas.is_pausing():
                    enable = True
        return enable
...
class ActionSelectFile(Action):
    def __init__(self, icon=None, title="", parent=None, action=None, is_enabled=None):
        super().__init__(
